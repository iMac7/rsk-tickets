import { BrowserProvider, Contract, Interface, JsonRpcProvider, Wallet } from "ethers";
...
export const USE_LOCAL_SIGNER = !TESTNET_RPC_URL;
export const READ_RPC_URL = TESTNET_RPC_URL || LOCAL_RPC_URL;
export const USE_SUBGRAPH = Boolean(!USE_LOCAL_SIGNER && SUBGRAPH_URL);
...
export async function connectWallet(accountNode: HTMLDivElement) {
  if (USE_LOCAL_SIGNER) {
...
export function disconnectWallet(accountNode: HTMLDivElement) {
  if (USE_LOCAL_SIGNER) {
    return;
  }
...
export async function initializeAccount(accountNode: HTMLDivElement) {
  if (USE_LOCAL_SIGNER) {
...
export async function getFactory(withSigner = false) {
  if (!FACTORY_ADDRESS) {
...
export function setActionError(address: string, message = "") {
...
export function clearActionError(address: string) {
...
export function getReadonlyAccount() {
  return state.account || ZERO_ADDRESS;
}
